...
# from branchflow.opf.MatlabFun import dfsearch, find
...
class PModel:
        ...
        for ph in "abc":
            if table[ph].shape[0] > 0:
                ...
                for i in range(self.der_bus[ph].shape[0]):
                    x_lim_lower[self.der_start_phase_idx[ph] + i] = 0
                    x_lim_upper[self.der_start_phase_idx[ph] + i] = p["g"][ph][
                        self.der_bus[ph][i]
                    ]
        ...
        for j in range(1, self.nb):
            ...
            for ph in "abc":
                if phase_exists[ph]:
                    ...
                    b_eq[p_row[ph]] = (1 - (cvr_p / 2)) * self.p["l"][ph][j]
                    # Q equation
    ...
    def constraints(self, xk):
        ...
        for j in range(1, self.nb):
            ...
            qg_a = 0  # letting q from DGs be always zero
            qg_b = 0
            qg_c = 0
            pg_a = 0
            pg_b = 0
            pg_c = 0
            if j in der_a_buses:
                pg_a_idx = int(self.der_start_phase_idx["a"] + find(j == der_a_buses))
                pg_a = xk[pg_a_idx]
            if j in der_b_buses:
                pg_b_idx = int(self.der_start_phase_idx["b"] + find(j == der_b_buses))
                pg_b = xk[pg_b_idx]
            if j in der_b_buses:
                pg_c_idx = int(self.der_start_phase_idx["c"] + find(j == der_c_buses))
                pg_c = xk[pg_c_idx]
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            if a_exists:
                ...
                # eqn 27
                g[p_a_row] = (
                    paa[i, j] - sum([(paa[j, k]) for k in k_buses_a]) - pl_a + pg_a
                )
                # eqn 28
                g[q_a_row] = (
                    qaa[i, j]
            ...
            if b_exists:
                ...
                # eqn 27
                g[p_b_row] = (
                    pbb[i, j] - sum([(pbb[j, k]) for k in k_buses_b]) - pl_b + pg_b
                )
                # eqn 28
                g[q_b_row] = (
                    qbb[i, j]
            ...
            if c_exists:
                ...
                # eqn 27
                g[p_c_row] = (
                    pcc[i, j] - sum([(pcc[j, k]) for k in k_buses_c]) - pl_c + pg_c
                )
                # eqn 28
                g[q_c_row] = (
                    qcc[i, j]
    ...
    def solve(self, objective, target=None, gld_correction=False, loss_percent=None):
        ...
        elif objective == "quadratic curtail":
            res = self.solve_quad(self.cp_obj_curtail)
        elif objective == "loss":
        ...
        #     l_residual_lin, u_residual_lin = self.find_inequality_residuals(
        #         x_prev, self.bounds
        #     )
        #     l_residual, u_residual = self.find_inequality_residuals(res.x, self.bounds)
        #     res.upper.residual = u_residual
        #     res.lower.residual = l_residual
        #     violation_adjusted = self.adjust_v_bounds(res.x)
        #     if violation_adjusted:
        #         print("violation adjustment---retry")
        #         self.x0 = x_prev
        #         res = self.solve(
        #             objective,
        #             target=target,
        #             gld_correction=gld_correction,
        #             loss_percent=loss_percent,
        #         )
        return res

    # def gld_solve(self, xk, p_base=None, v_ll_base=None, dir_path=None):
    #     p_gen = self.get_dec_variables(xk)
...
if __name__ == "__main__":
    ...
    #
    # for area in ['area1']:
    #     area_dir = Path(f"/home/nathangray/PycharmProjects/CPMACosim_T2/gridlabd/10_ders/{area}")
    #     assert area_dir.exists()
    #     # area_dir = Path("/home/nathangray/PycharmProjects/glmpy_dev/glmpy/unittest/epb_test/gridlabd/epb_feeders/converted/TEN207")
    #     branchdata = get_branchdata(area_dir)
    #     powerdata = get_powerdata(area_dir)
    #
    #     # branchdata.iloc[:, 2:14] /= (4.16 / np.sqrt(3)) ** 2
    #     # powerdata.iloc[:, 1:13] /= 1000
    #     m = PModel(
    #         branchdata,
    #         powerdata,
    #         v_up=1.035,
    #         n_children=0,
    #         # Real power rating as a multiple of power listed in powerdata
    #         # Apparent power rating of inverters as a multiple of the real power rating (typ. 1.2)
    #         gen_mult=1.0,  # Multiply the inverter outputs by this. !Does not effect real power rating!
    #         load_mult=1.0,  # Multiply the loads by this
    #         p_rating_mult=1,
    #         gld_dir=area_dir,
    #         # Provide an existing directory where a new directory with the new glm will be made and run.
    #         p_base_gld=1e6,  # When the GLM is created powerdata will be multiplied by this.
    #         v_ll_base_gld=4160,  # GLM SWING bus voltage and impedance will use this to convert to volts and ohms resp.
    #     )
    #     res = m.solve(objective='p_target', target=0.7, gld_correction=False)
    #     all_v_sq, all_s, dec_var, _ = m.parse_output(res.x)
    #     print("load")
    #     print(all_s[:, 0])
    #     print("sum")
    #     print(all_s[:, 0].real.sum())
    #     print(dec_var)

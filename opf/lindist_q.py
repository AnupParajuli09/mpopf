...
class LinDistModelQ:
    ...
    def __init__(
        ...
        self.bat = bat_data.sort_values(by="id", ignore_index = True)
        self.loadshape = loadshape_data.sort_values(by="time", ignore_index = True)
        self.pv_loadshape = pv_loadshape_data.sort_values(by="time", ignore_index=True)
        self.bus.index = self.bus.id - 1
        self.gen.index = self.gen.id - 1
        self.cap.index = self.cap.id - 1
        self.bat.index = self.bat.id - 1
        self.loadshape.index = self.loadshape.time
        self.pv_loadshape.index = self.pv_loadshape.time
        ...
        self.battery_bus = {
            "a": self.bat.phases.str.contains("a").index.to_numpy(),
            "b": self.bat.phases.str.contains("b").index.to_numpy(),
            "c": self.bat.phases.str.contains("c").index.to_numpy(),
        }
    ...
    def init_bounds(self, bus, gen, bat, pij_max=100e3):
        x_maps = self.x_maps
            ...
            for j in range(1, self.nb):
                ...
                for ph in ["abc", "bca", "cab"]:
                    ...
                    a_eq[row("pij", a), col("vj", a)] = (
                            -(bus.cvr_p[j] / 2) * bus["pl_" + a][j] * loadshape["M"][t]
                    )
                    a_eq[row("pij", a), children("pij", a)] = -1
                    b_eq[row("pij", a)] = (1 - (bus.cvr_p[j] / 2)) * bus["pl_" + a][j] * loadshape["M"][t]
                    # Q equation
                    ...
                    if LinDistModelQ.der:
                        ...
                        b_eq[row("pij", a)] = (1 - (bus.cvr_p[j] / 2)) * bus["pl_" + a][j] * loadshape["M"][t] - g["p" + a].get(j, 0) * pv_shape["PV"][t]
                        b_eq[row("qij", a)] = (1 - (bus.cvr_q[j] / 2)) * bus["ql_" + a][
                            j
                        ] * loadshape["M"][t] - cap["q" + a].get(j, 0)
                    if LinDistModelQ.battery:
                        ...
                        a_eq[row("b", a), col("pc", a)] = -bat["nc_" + a].get(j, 0)
                        a_eq[row("b", a), col("pd", a)] = 1 / bat["nd_" + a].get(j, float('inf'))
                        if t == 0:
                            b_eq[row("b", a)] = 0.5 * bat["Pb_max_" + a].get(j,0)
                        else:
                            b_eq[row("b", a)] = 0
                        ...
                        b_ineq[row("pd", a)] = bat["hmax_" + a].get(j,0)
    ...
    def get_q_dec_variables(self, x_sol):
        ...
        if LinDistModelQ.der and LinDistModelQ.battery:
            ...
            for t in range(LinDistModelQ.n):
                ...
                for ph in "abc":
                    for j in range(ng[ph]):
                        dec_var_t.loc[self.der_bus[ph][j]+1, ph] = x_sol[qi[t][ph] + j]
                        dec_d_var_t.loc[self.battery_bus[ph][j]+1, ph] = x_sol[pdi[t][ph] + j]
                        dec_c_var_t.loc[self.battery_bus[ph][j]+1, ph] = x_sol[pci[t][ph] + j]
                        dec_b_var_t.loc[self.battery_bus[ph][j]+1, ph] = x_sol[bi[t][ph] + j]
                dec_var[t] = dec_var_t
